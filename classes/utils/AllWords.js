...
export class AllWords {
	...
	static ALL_WORDS = Object.freeze(
		[
			{
				"category": "Number",
				"cname": "数字",
				"words": [
					{ "chinese": "一", "english": "One", "pinyin": "Yī", "phrase": ["一个", "一对", "一心一意"], "sentence": "一个小孩开飞机。", "image": "images/chars/Number/number-1.png" },
					{ "chinese": "二", "english": "Two", "pinyin": "Ér", "phrase": ["二人", "二斤", "合二为一"], "sentence": "两个小孩梳小辫。", "image": "images/chars/Number/number-2.png" },
					{ "chinese": "三", "english": "Three", "pinyin": "Sān", "phrase": ["三只", "三天", "三心二意"], "sentence": "三个小孩吃饼干。", "image": "images/chars/Number/number-3.png" },
					{ "chinese": "四", "english": "Four", "pinyin": "Sì", "phrase": ["四把", "四个月", "四平八稳"], "sentence": "四个小孩写大字。", "image": "images/chars/Number/number-4.png" },
					{ "chinese": "五", "english": "Five", "pinyin": "Wǔ", "phrase": ["五辆", "五台", "五谷丰登"], "sentence": "五个小孩烤白薯。", "image": "images/chars/Number/number-5.png" },
					{ "chinese": "六", "english": "Six", "pinyin": "Liù", "phrase": ["六条", "六周", "六六大顺"], "sentence": "六个小孩烧牛肉。", "image": "images/chars/Number/number-6.png" },
					{ "chinese": "七", "english": "Seven", "pinyin": "Qī", "phrase": ["七匹", "七头", "七上八下"], "sentence": "七个小孩穿新衣。", "image": "images/chars/Number/number-7.png" },
					{ "chinese": "八", "english": "Eight", "pinyin": "Bā", "phrase": ["八张", "八回", "四面八方"], "sentence": "八个小孩吹喇叭。", "image": "images/chars/Number/number-8.png" },
					{ "chinese": "九", "english": "Nine", "pinyin": "Jiǔ", "phrase": ["九块", "九磅", " 八九不离十"], "sentence": "九个小孩看杨柳。", "image": "images/chars/Number/number-9.png" },
					{ "chinese": "十", "english": "Ten", "pinyin": "Shí", "phrase": ["十双", "十章", "十拿九稳"], "sentence": "十个小孩不挑食。", "image": "images/chars/Number/number-10.png" },
			...
			},
			{
				...
				"words": [
					...
					{ "chinese": "姐", "english": "Elder-Sister", "pinyin": "Jiě", "phrase": ["姐姐", "姐妹"], "sentence": "我是姐姐，我的弟弟叫胡文思。", "image": "images/chars/Family/elder-sister.jpg" },
					...
					{ "chinese": "妹", "english": "Younger-Sister", "pinyin": "Mèi", "phrase": ["妹妹", "姐妹"], "sentence": "我没有妹妹，但我有个弟弟。", "image": "images/chars/Family/younger-sister.webp" }
				]
			},
			{
				"category": "Body",
	...
	static GREETING_IMG_DIR = 'images/greetings/';
	static GREETING_IMAGES = [
		{
			category: 'All',
	...
	static getGreetingImages() {
		...
		for (let i = 0; i < this.GREETING_IMAGES.length; i++) {
			greetingImage = this.GREETING_IMAGES[i];
			image = this.createImg(greetingImage.category, greetingImage.wrong, 'wrong');
			images.push(image);
			image = this.createImg(greetingImage.category, greetingImage.ok, 'ok');
			images.push(image);
			for (let j = 0; j < greetingImage.great.length; j++) {
				image = this.createImg(greetingImage.category, greetingImage.great[j], 'great' + j);
				images.push(image);
			}
		}
		return images;
	...
	/**
	 * Creates a greeting image for preloading
	 * @param {*} category 
	 * @param {*} url 
	 * @param {*} name 
	 * @returns 
	 */
	static createImg(category, url, name) {
		let image = document.createElement('IMG');
		image.id = category + '-' + name;
		image.src = this.GREETING_IMG_DIR + url;
		image.className = "recording";
	...
	static setCurrentWord3(category, cname, word) {
		this.CURRENT_WORD.category = category;
		this.CURRENT_WORD.cname = cname;
		this.CURRENT_WORD.word = word;
	}

	static setCurrentWord1(word) {
	 ...
	 * @returns CURRENT_WORD {category, cname, word} if dound, otherwise null;
	 */
	static findWord(searchWord) {
		let category, word = null;
		for (let i = 0; i < this.ALL_WORDS.length && word == null; i++) {
			category = this.ALL_WORDS[i];
			word = category.words.find(
	 ...
	 * Gets a word by Chinese or English from the allWord list and sets it as the current word.
	 * Note that if the word is not unique, return the first one found.
	 * @returns CURRENT_WORD {category, cname, word}
	 */
	static findAndSetWord(searchWord) {
		let category, word = null;
		for (let i = 0; i < this.ALL_WORDS.length && word == null; i++) {
			category = this.ALL_WORDS[i];
			word = category.words.find(
	 ...
	 * @returns CURRENT_WORD {category, cname, word}
	 */
	static findAndSetWord2(searchWord1, searchWord2) {
		let category, word;
		for (let i = 0; i < this.ALL_WORDS.length && word == null; i++) {
			category = this.ALL_WORDS[i];
			word = category.words.find(item =>
	 ...
	 * @param {*} names 
	 */
	static findAndSetWord3(names) {
		let word;
		let category = AllWords.getCategory(names[0]);
		...
		switch (names.length) {
			...
			case 2:  // category and the English or Chinese word
				word = category.words.find(item => item.english == names[1] || item.chinese == names[1]);
				break;
	...
	static getCategory(categoryName) {
		let category = this.ALL_WORDS.find(item => item.category == categoryName);
		if (category == null) {
			category = this.ALL_WORDS[0];
		}
		return category;
	 ...
	 * @returns 
	 */
	static getRandomImage(category) {
		let images = this.GREETING_IMAGES.find(item => item.category == category);
		if (images === undefined) {
			images = this.GREETING_IMAGES.find(item => item.category == 'All');
		}
		let min = Math.ceil(0);
